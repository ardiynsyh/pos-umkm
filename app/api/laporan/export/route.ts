...
const mapPaymentMethod = (method: string) => {
  const map: Record<string, string> = {
    TUNAI: 'Tunai', DEBIT: 'Kartu Debit', KREDIT: 'Kartu Kredit', QRIS: 'QRIS',
    TRANSFER: 'Transfer', cash: 'Tunai', qris: 'QRIS', transfer: 'Transfer',
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // ── Buat workbook (sama seperti sebelumnya) ──────────────────────────────
    const wb = new ExcelJS.Workbook();
    wb.creator = 'POS UMKM';
    wb.created = new Date();
    const ws = wb.addWorksheet('Laporan Transaksi', { pageSetup: { fitToPage: true, orientation: 'landscape' } });

    const filterLabel: Record<string, string> = { today: 'Hari Ini', week: '7 Hari Terakhir', month: '30 Hari Terakhir', custom: 'Periode Custom' };
    ws.mergeCells('A1:K1');
    ...
    const HEADER_ROW = 4;
    const headers = [
      { header: 'No Transaksi', width: 22 }, { header: 'Sumber', width: 16 },
      { header: 'Tanggal', width: 20 }, { header: 'Kasir / Customer', width: 20 },
      { header: 'Meja', width: 10 }, { header: 'Produk', width: 28 },
      { header: 'Qty', width: 8 }, { header: 'Harga Satuan (Rp)', width: 20 },
      { header: 'Subtotal Produk (Rp)', width: 22 }, { header: 'Total Transaksi (Rp)', width: 22 },
    ...
    const rupiahFmt = '#,##0';
    const borderStyle: Partial<ExcelJS.Borders> = { top: { style: 'thin', color: { argb: 'FFE5E7EB' } }, bottom: { style: 'thin', color: { argb: 'FFE5E7EB' } }, left: { style: 'thin', color: { argb: 'FFE5E7EB' } }, right: { style: 'thin', color: { argb: 'FFE5E7EB' } } };

    const styleDataRow = (row: ExcelJS.Row, isAlt: boolean) => {
      const bg = isAlt ? 'FFF0F4FF' : 'FFFFFFFF';
      row.eachCell({ includeEmpty: true }, (cell, colNum) => {
        cell.font = { name: 'Arial', size: 10 }; cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: bg } }; cell.border = borderStyle;
    ...
    let rowIndex = HEADER_ROW + 1, altToggle = false, grandTotal = 0;

    for (const t of transactions) {
      grandTotal += t.total;
      for (const item of t.items) {
        const row = ws.getRow(rowIndex++);
        row.values = [t.nomorTransaksi, 'Kasir', formatDate(t.createdAt), t.kasir || 'Kasir', '-', item.namaProduk, item.quantity, item.hargaSatuan, item.subtotal, t.total, mapPaymentMethod(t.metodePembayaran)];
        [8, 9, 10].forEach((col) => { row.getCell(col).numFmt = rupiahFmt; });
        styleDataRow(row, altToggle); altToggle = !altToggle;
      }
    }
    for (const o of orders) {
      ...
      for (const item of o.items) {
        const row = ws.getRow(rowIndex++);
        row.values = [o.orderNumber, 'Customer Order', formatDate(o.createdAt), o.customerName || 'Guest', o.table?.label || '-', item.productName, item.quantity, item.price, item.subtotal, o.totalAmount, mapPaymentMethod(o.paymentMethod || 'cash')];
        [8, 9, 10].forEach((col) => { row.getCell(col).numFmt = rupiahFmt; });
        styleDataRow(row, altToggle); altToggle = !altToggle;
      }
    }
    ...
    const filterSlug: Record<string, string> = { today: 'hari-ini', week: '7-hari', month: '30-hari', custom: 'custom' };
